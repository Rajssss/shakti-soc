# C-Class on NEXYS-VIDEO Board

This repo contains the flow for porting an instance of the C-class on the [NEXYS-VIDEO](https://store.digilentinc.com/arty-a7-artix-7-fpga-development-board-for-makers-and-hobbyists/) board from digilent. 
...
## Quick Start (default Config) :: Get started with an NEXYS-VIDEO
...
cd shakti-soc/fpga/boards/nexysvideo/c-class
...
2. Change present working directory to *shakti-soc/fpga/boards/nexysvideo/c-class*.
```
$ cd shakti-soc/fpga/boards/nexysvideo/c-class
```
...
7. Synthesize, implement and generate the bitstream (The bitstream is generated at *shakti-soc/fpga/boards/nexysvideo/c-class/fpga_project/c-class/c-class.runs/core_impl_1/fpga_top.bit*)
```
$ make arty_build
```

8. Connect the Nexys Video board.

9. Program the FPGA with the generated bitstream (Requires sudo access)*
```
$ make program
```
\* The "*make program*" command creates a folder whose owner is root. Therefore, before running "*make arty_build*" command again, delete the *shakti-soc/fpga/boards/nexysvideo/c-class/fpga_project/c-class* folder.
...
## Reporting Issues
While reporting issues with this board/build please make sure to mark the issues with the `cclass-nexys-video` label
